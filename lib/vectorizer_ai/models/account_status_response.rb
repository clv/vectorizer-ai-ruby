  ...
  class AccountStatusResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VectorizerAI::AccountStatusResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VectorizerAI::AccountStatusResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @subscription_state.nil?
        invalid_properties.push('invalid value for "subscription_state", subscription_state cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @subscription_state.nil?
    ...
    def hash
      [subscription_plan, subscription_state, credits].hash
    end
    ...
    def self._deserialize(type, value)
      ...
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        ...
        klass = VectorizerAI.const_get(type)
